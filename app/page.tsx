    ...
    return (
      <main className="min-h-screen p-4 md:p-8 max-w-2xl mx-auto">
        ...
        <div className="card-elevated p-6 mb-6 text-center">
          <div style={{ fontSize: "3rem", lineHeight: 1 }} className="mb-2">
          ...
          <div className="font-mono text-xs" style={{ color: "var(--text-muted)" }}>
            TOP {result.rarityPercentile}% OF WALLETS SCANNED
          </div>
        </div>

        {/* Scores */}
        <div className="grid grid-cols-2 gap-3 mb-6" style={{ gridTemplateColumns: result.scores.social !== null ? "1fr 1fr 1fr 1fr" : "1fr 1fr 1fr" }}>
        ...
        <div className="flex flex-wrap gap-2 mb-6">
          {result.verdictTags.map((tag) => (
            <span
              key={tag}
              className={`tag ${tag.includes("⚠️") || tag.includes("Critical") ? "tag-critical" : ""}`}
            >
              {tag}
            </span>
          ))}
        ...
        <div className="card mb-4" style={{ borderColor: "rgba(245, 166, 35, 0.4)" }}>
          <div className="font-mono text-xs mb-2" style={{ color: "var(--amber)" }}>
            🔥 ROAST
          </div>
          <p className="text-sm leading-relaxed italic" style={{ color: "var(--phosphor)" }}>
            {result.roast}
          </p>
        </div>
        ...
        <div className="card mb-6">
          ...
          >
            * On-chain ERC-20 approvals only. Off-chain signatures (Permit2,
            EIP-712) not covered.
          </div>
        </div>
          ...
          <div className="card mb-6">
            ...
            <div className="flex items-center gap-4 text-sm font-mono">
              <span style={{ color: "var(--amber)" }}>
                @{result.socialData.farcasterUsername}
              </span>
              <span style={{ color: "var(--text-muted)" }}>
                {result.socialData.followerCount} followers
              </span>
              <span style={{ color: "var(--text-muted)" }}>
                {result.socialData.castCount} casts
              </span>
            </div>
          </div>
        )}
        ...
        >
          Scanned via {result.dataSource.toUpperCase()} ·{" "}
  ...
  return (
    <main className="min-h-screen flex flex-col items-center justify-center p-6">
      ...
      <div className="mb-12 text-center">
        <div
          className="font-mono font-bold glow-amber"
          style={{ fontSize: "clamp(2rem, 8vw, 3.5rem)", color: "var(--amber)", letterSpacing: "0.15em" }}
        >
          BASEIQ
        </div>
        <div
          className="font-mono text-xs mt-2 tracking-widest"
          style={{ color: "var(--text-muted)" }}
        >
      ...
      {/* Mini App prefill — only shown inside Base App / Farcaster */}
      {isMiniApp && viewerAddress && (
        <button
          className="btn-ghost mb-4"
          onClick={() => handleScan(viewerAddress)}
        >
      ...
      {/* Footer */}
      <div
        className="absolute bottom-6 text-xs font-mono text-center"
        style={{ color: "var(--text-muted)" }}
      >
